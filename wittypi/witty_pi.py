...
class WittyPi():
	...
	def __repr__(self) -> str:
		return "Witty Pi (State: {}, {}, {}, Available Schedules: {})".format(self.path, self.next_start, self.next_shutdown, self.available_schedules)
	...
	def _check_hardware(self):
		for file in self.required_files:
			if not os.path.isfile(os.path.join(self.path, file)):
				raise Exception("Witty Pi script '{}' does not exist in '{}'.".format(file, self.path))
		if self.__run_util_function("is_rtc_connected")[0] == 1:
			raise Exception("Witty Pi is not connected.")
		self._get_startup()
		self._get_shutdown()
	...
	def _list_schedules(self):
		schedules = []
		for file in os.listdir(os.path.join(self.path, "schedules")):
			if file.endswith(".wpi"):
				schedules.append(file)
		self.available_schedules = sorted(schedules)
